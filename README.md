...
insert 20000 facts                                                 0.69
export 20000 facts                                                 0.02
import 411050 bytes (20000 facts)                                  0.02
insert 10 facts                                                    0.04
query 10 times w/txn                                               2.52
query 10 times w/o txn                                             0.10
modify 10 attrs w/txn                                              2.22
delete 10 facts w/txn                                              3.24
...
query 5000 facts building index on-the-fly                         0.12
...
transaction rollback on factbase with 100000 facts                 0.17
(gt time '2024-03-23T03:21:43Z')                                   0.37
(gt cost 50)                                                       0.18
(eq title 'Object Thinking 5000')                                  0.06
(and (eq foo 42.998) (or (gt bar 200) (absent z...                 0.04
(and (exists foo) (not (exists blue)))                             1.72
(eq id (agg (always) (max id)))                                    2.97
(join "c<=cost,b<=bar" (eq id (agg (always) (ma...                 4.63
(and (eq what "foo") (join "w<=what" (and (eq i...                 7.42
delete!                                                            0.53
(and (eq issue *) (eq repository *) (eq what '*') (eq where '*'))  0.44
Taped.append() x50000                                              0.02
Taped.each() x125                                                  1.67
Taped.delete_if() x375                                             0.85
50000 facts: read-only txn (no copy needed)                        7.73
50000 facts: rollback txn (no copy needed)                         7.22
50000 facts: insert in txn (copy triggered)                        3.27
50000 facts: modify in txn (copy triggered)                       38.04
100000 facts: read-only txn (no copy needed)                      16.65
100000 facts: rollback txn (no copy needed)                       16.43
100000 facts: insert in txn (copy triggered)                       7.93
100000 facts: modify in txn (copy triggered)                      74.85
```

The results were calculated in [this GHA job][benchmark-gha]
on 2025-12-12 at 13:37,
on Linux with 4 CPUs.
<!-- benchmark_end -->

[benchmark-gha]: https://github.com/HaidarJbeily7/factbase/actions/runs/20168405340
